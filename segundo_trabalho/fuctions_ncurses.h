...
string writer(WINDOW *window, char *node_char, const char *text, int x, int y){
    ...
    attron(COLOR_PAIR(1)); getstr(node_char); string node_string = node_char;

    while (node_string == ""){
        mvprintw(28, 44, " Preencha o campo para prosseguir. ");
        getstr(node_char); node_string = node_char;}
...
bool quit(const char *text, int textx, int texty){
    ...
    choice = 0; highlight = 1; const char *choices[] = {"       Sim        ", "       Não        "};
    int n_choices = sizeof(choices)/sizeof(char *);
